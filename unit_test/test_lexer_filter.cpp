...
TEST(test_parser_lexer, lexer_filter)
{
    ...
    // int
    result = lexer_filter_group_.filterate(char_stream_);
    EXPECT_EQ(filter_status::filtered, result.first);
    EXPECT_EQ(token_type(TOK_KEYWORD_INT), static_cast<token_type&>(result.second));
    EXPECT_EQ(std::string("int"), result.second.to_string());
    // deliminator
    result = lexer_filter_group_.filterate(char_stream_);
    EXPECT_EQ(filter_status::filtered, result.first);
    EXPECT_EQ(token_type(TOK_DELIMINATORS), static_cast<token_type&>(result.second));
    EXPECT_EQ(std::string(" "), result.second.to_string());
    // fun
    result = lexer_filter_group_.filterate(char_stream_);
    EXPECT_EQ(filter_status::filtered, result.first);
    EXPECT_EQ(token_type(TOK_STRING), static_cast<token_type&>(result.second));
    EXPECT_EQ(std::string("fun"), result.second.to_string());
    // (
    result = lexer_filter_group_.filterate(char_stream_);
    EXPECT_EQ(filter_status::filtered, result.first);
    EXPECT_EQ(token_type(TOK_LP), static_cast<token_type&>(result.second));
    EXPECT_EQ(std::string("("), result.second.to_string());
    // i
    result = lexer_filter_group_.filterate(char_stream_);
    EXPECT_EQ(filter_status::filtered, result.first);
    EXPECT_EQ(token_type(TOK_STRING), static_cast<token_type&>(result.second));
    EXPECT_EQ(std::string("i"), result.second.to_string());
    // ,
    result = lexer_filter_group_.filterate(char_stream_);
    EXPECT_EQ(filter_status::filtered, result.first);
    EXPECT_EQ(token_type(TOK_COMMA), static_cast<token_type&>(result.second));
    EXPECT_EQ(std::string(","), result.second.to_string());
    // deliminator
    result = lexer_filter_group_.filterate(char_stream_);
    EXPECT_EQ(filter_status::filtered, result.first);
    ...
    EXPECT_EQ(filter_status::filtered, result.first);
    EXPECT_EQ(token_type(TOK_STRING), static_cast<token_type&>(result.second));
    EXPECT_EQ(std::string("j"), result.second.to_string());
    // )
    result = lexer_filter_group_.filterate(char_stream_);
    EXPECT_EQ(filter_status::filtered, result.first);
    EXPECT_EQ(token_type(TOK_RP), static_cast<token_type&>(result.second));
    EXPECT_EQ(std::string(")"), result.second.to_string());
    // ;
    result = lexer_filter_group_.filterate(char_stream_);
    EXPECT_EQ(filter_status::filtered, result.first);
    EXPECT_EQ(token_type(TOK_SEMICOMMA), static_cast<token_type&>(result.second));
    EXPECT_EQ(std::string(";"), result.second.to_string());
    // eof
    result = lexer_filter_group_.filterate(char_stream_);
    EXPECT_EQ(filter_status::eof, result.first);
}
